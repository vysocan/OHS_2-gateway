...
/*
 * Send time beacon every hour
 */
static void serviceTimeBeacon(time_t timeNow, uint16_t counter, uint8_t *message) {
...
/*
 * Remove zombie nodes that have not responded for an hour
 */
static void serviceNodeHousekeeping(time_t timeNow) {
 ...
 */
static void serviceBatteryCheck(void) {
 ...
 */
static void serviceACPowerCheck(uint8_t *counterAC, bool *flagAC) {
  msg_t resp = palReadPad(GPIOD, GPIOD_AC_OFF);
...
/*
 * Group auto arm - arm groups whose zones have been inactive
 */
static void serviceGroupAutoArm(time_t timeNow) {
  time_t tempTime;
...
/*
 * Zone open alarm - alert when zone has been open too long
 */
static void serviceZoneOpenAlarm(time_t timeNow) {
  for (uint8_t zoneNum = 0; zoneNum < ALARM_ZONES; zoneNum++) {
...
/*
 * Process timer on/off events and script evaluation
 */
static void serviceTimers(time_t timeNow, uint8_t *message) {
  uint8_t nodeIndex;
  char   *pResult;
...
/*
 * Trigger Off timer - send off packets and log when trigger timer expires
 */
static void serviceTriggerOff(time_t timeNow, uint8_t *message) {
  uint8_t nodeIndex;
...
static THD_FUNCTION(ServiceThread, arg) {
  ...
  while (true) {
    ...
    serviceTimeBeacon(timeNow, counter, message);
    serviceNodeHousekeeping(timeNow);
    serviceBatteryCheck();
    serviceACPowerCheck(&counterAC, &flagAC);
