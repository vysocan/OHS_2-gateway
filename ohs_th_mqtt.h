...
static THD_FUNCTION(MqttSubThread, arg) {
  ...
  while (true) {
    ...
    if (msg == MSG_OK) {
      ...
      // Decode topic string
      savePtr = NULL;
      pch = strtok_r(&inMsg->topic[0], "/" , &savePtr); // Main topic
      ...
      if (pch != NULL) {
        ...
        // Sensors
          ...
          while ((pch != NULL) && (index < NODE_ADDRESS_SIZE)) {
            ...
            index++;
            if (index < (NODE_ADDRESS_SIZE - 1)) pch = strtok_r(NULL, ":" , &savePtr);
            else pch = strtok_r(NULL, "/" , &savePtr);
          }
          if (index == NODE_ADDRESS_SIZE) {
            ...
            if ((index != DUMMY_NO_VALUE)
              ...
              // strtok_r done already while getting address parts
              if (pch != NULL) {
                ...
                if ((strcmp (pch, text_value) == 0)
                  ...
                  // ToDo make simple safe functions for safe_strtof() / safe_strtoul()
